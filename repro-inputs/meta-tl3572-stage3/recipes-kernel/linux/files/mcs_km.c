...
extern int arm64_mcs_ipi_virq(unsigned int index);
extern void arm64_mcs_ipi_send(unsigned int index,
				const struct cpumask *target);

static int mcs_irq[MCS_CLIENT_COUNT] = { -1, -1 };
static int __percpu *mcs_evt[MCS_CLIENT_COUNT];
static const char * const mcs_ipi_name[MCS_CLIENT_COUNT] = {
	"MCS IPI8", "MCS IPI9"
};
...
static irqreturn_t handle_clientos_ipi(int irq, void *data)
{
	...
	pr_debug("received ipi %d from client os\n", irq);
	for (i = 0; i < MCS_CLIENT_COUNT; i++) {
		if (irq == mcs_irq[i]) {
			atomic_or(BIT(i), &irq_pending);
			break;
		}
	}
...
static void enable_mcs_ipi(void *data)
{
	...
	for (i = 0; i < MCS_CLIENT_COUNT; i++)
		enable_percpu_irq(mcs_irq[i], IRQ_TYPE_NONE);
}
...
static void remove_mcs_ipi(void)
{
	...
	for (i = 0; i < MCS_CLIENT_COUNT; i++) {
		free_percpu_irq(mcs_irq[i], mcs_evt[i]);
		free_percpu(mcs_evt[i]);
		mcs_evt[i] = NULL;
		mcs_irq[i] = -1;
	}
}
...
static int init_mcs_ipi(void)
{
	...
	for (i = 0; i < MCS_CLIENT_COUNT; i++) {
		mcs_irq[i] = arm64_mcs_ipi_virq(i);
		if (mcs_irq[i] < 0) {
			err = mcs_irq[i];
			pr_err("failed to resolve MCS IPI%u, ret = %d\n", i, err);
			goto err_free;
		}

		irq_data = irq_get_irq_data(mcs_irq[i]);
		if (!irq_data) {
			pr_err("no irq data for IRQ %d\n", mcs_irq[i]);
			err = -EINVAL;
			goto err_free;
		}
		...
		mcs_evt[i] = alloc_percpu(int);
		if (!mcs_evt[i]) {
			err = -ENOMEM;
			goto err_free;
		}

		err = request_percpu_irq(mcs_irq[i], handle_clientos_ipi,
					 mcs_ipi_name[i], mcs_evt[i]);
		if (err) {
			free_percpu(mcs_evt[i]);
			mcs_evt[i] = NULL;
			goto err_free;
...
static int mcs_ipi_index(u32 cpu)
{
	if (cpu < MCS_CLIENT_CPU_BASE ||
	    cpu >= MCS_CLIENT_CPU_BASE + MCS_CLIENT_COUNT)
		return -EINVAL;

	return cpu - MCS_CLIENT_CPU_BASE;

}

static int send_clientos_ipi(u32 cpu)
{
	int index = mcs_ipi_index(cpu);

	if (index < 0)
		return index;

	arm64_mcs_ipi_send(index, cpumask_of(cpu));
	return 0;
}
...
static unsigned int mcs_poll(struct file *file, poll_table *wait)
{
	...
	pending = atomic_xchg(&irq_pending, 0);
	if (pending & BIT(0))
