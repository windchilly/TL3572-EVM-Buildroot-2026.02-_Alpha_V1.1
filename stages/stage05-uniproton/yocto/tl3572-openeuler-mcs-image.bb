...
TL3572_UNIPROTON_SHA256 = "3faa550c98c18a407e1a2816b3c022c82fc91d85b3fb968b83ffed32631478d0"
SRC_URI += " \
...
tl3572_bsp_rootfs_config () {
    ...
    # Board-specific MCS runtime.  Keep only the TL3572 instance so micad does
    # not probe sample QEMU/RPi configs whose firmware is absent.
    rm -f ${IMAGE_ROOTFS}/etc/mica/*.conf
    install -m 0644 ${THISDIR}/files/tl3572-up0-autoboot.conf \
        ${IMAGE_ROOTFS}/etc/mica/tl3572-up0.conf
    install -m 0644 ${THISDIR}/files/rk3572-uniproton.elf \
        ${IMAGE_ROOTFS}/lib/firmware/rk3572-uniproton.elf
    firmware_sha256=$(sha256sum \
        ${IMAGE_ROOTFS}/lib/firmware/rk3572-uniproton.elf | awk '{print $1}')
    if [ "$firmware_sha256" != "${TL3572_UNIPROTON_SHA256}" ]; then
        bbfatal "TL3572 UniProton firmware checksum mismatch: $firmware_sha256"
    fi
